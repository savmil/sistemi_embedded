...
static irq_handler_t isr_handler(int irq,void *dev_id){
	...
	switch(irq){
		case 46:
			printk(KERN_INFO"**********ISR SWITCH***********");
			break;
		case 47:
			printk(KERN_INFO"**********ISR BUTTON***********");
			break;
		case 48:
...
/* Write operation for /proc/GPIO_0
* --------------------------------------
*/
...
/* Callback function when opening file /proc/GPIO_0
* ------------------------------------------------------
*/
static int proc_GPIO_show(struct seq_file *p, void *v){
	...
	read_reg = ioread32(v + READ_OFF); // leggo valore da READ register
...
/* File Operations for /proc/GPIO_0 */
...
static int __test_int_driver_probe(struct platform_device *pdev){
	...
	GPIO_proc_entry = proc_create(pdev->name, 0, NULL, &proc_GPIO_operations);
	...
	rval = request_irq(dm->irq, (irq_handler_t) isr_handler, IRQF_SHARED , pdev->name, dm->base_addr);
	if(rval != 0){
