...
int main(void){
	...
	XScuGic_SetPriorityTriggerType(&InterruptController,XPAR_FABRIC_MY_UART_INT_0_IRQ_INTR,0x0,0x3);
...
{
	count++;
	MYINTUART_GlobalDisableInterrupt(&myIntUARTInstance,0x0);
	MYINTUART_Stop(&myIntUARTInstance);
	InterruptProcessed = TRUE;
	pendingReg = MYINTUART_GetPending(&myIntUARTInstance);
	printf("PENDING REG %08x: ",pendingReg);

	/* NB nel caso reale dovrei fare else if perchè devo servirne una sola per ISR */

	if((pendingReg & 0x00000002 & INT_MASK) == 0x00000002)
		ISR_RX();
	else if((pendingReg & 0x00000001 & INT_MASK) == 0x00000001)
		ISR_TX();
